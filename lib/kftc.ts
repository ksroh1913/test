...
function getMockSummary(): AccountSummary {
  return {
...
async function callKftcApi(accessToken: string, authCode: string): Promise<KftcResponse> {
  ...
  try {
    ...
    return (await res.json()) as KftcResponse;
  } finally {
...
export async function fetchKftcAccounts(): Promise<AccountSummary> {
  ...
  const data = await callKftcApi(accessToken, authCode);
